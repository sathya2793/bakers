...
const ContactUs = () => {
  const [formData, setFormData] = useState({
    ...
    eventDate: '',
    customEvent: '',
    finalDescription: '' // Overall description of how they want the final cake
  });

  const [selectedFiles, setSelectedFiles] = useState([]);
  const [imagePreviews, setImagePreviews] = useState([]);
  const [imageDescriptions, setImageDescriptions] = useState([]); // Individual descriptions for each image
  const [loading, setLoading] = useState(false);

  const validateFile = (file) => {
    const allowedTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp'];
    if (!allowedTypes.includes(file.type)) {
      showCompactError(`"${file.name}" is not a valid image file. Please upload JPG, PNG, or WebP images.`);
      return false;
    }

    const maxSize = 5 * 1024 * 1024; // 5MB
    if (file.size > maxSize) {
      showCompactError(`"${file.name}" is too large (${(file.size / 1024 / 1024).toFixed(1)}MB). Maximum size is 5MB.`);
      return false;
    }
  ...
  const handleImageChange = (e) => {
    ...
    if (imagePreviews.length + files.length > 10) {
      showCompactWarning('Maximum 10 images allowed. Some files will be ignored.');
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (validationErrors.length > 0) {
      showCompactError(`Please fix the following: ${validationErrors.join(', ')}`);
      return;
    }
    ...
    // Add image descriptions as a JSON string
      ...
      <form onSubmit={handleSubmit} className="contact-form" onKeyDown={(e) => {
        ...
        <div className="form-section">
          ...
          <div className="form-row">
            ...
            <div className="form-group">
              <label>Mobile Number * <span className="helper-text">(Indian numbers only)</span></label>
        ...
        <div className="form-section">
          ...
          <p className="section-description">
            Upload images of cakes that inspire you. For each image, describe what specific elements you like about it.
          </p>
          ...
          <div className="file-upload-area">
            ...
            <div className="file-info">
              <small>📸 Upload up to 10 images • JPG, PNG, WebP • Max 5MB each</small>
            </div>
          </div>
